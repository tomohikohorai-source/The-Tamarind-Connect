      ...
      allow update: if (
        isSignedIn() && (
          ...
          (existing().userId == request.auth.uid && isValidMarketItem(incoming()) && 
           incoming().userId == existing().userId) || // userId is immutable
          
          // Action: Process Transaction (Buyer or Seller)
          (incoming().diff(existing()).affectedKeys().hasOnly([
            'requestStatus', 'buyerId', 'buyerNickname', 'buyerAvatarIcon', 
            'comments', 'likes', 'lastUpdated', 'status',
            'buyerConfirmedCompletion', 'sellerConfirmedCompletion', 
            'buyerRequestedCancellation', 'sellerRequestedCancellation'
